...
local MenuParameters = {
    ...
    CheatName = "Bloxsense",
    UserType = env.login and env.login.username or game.Players.LocalPlayer.Name,
...
do
    ...
    UILibrary:CreateSlider({Name = "Aimbot FOV", Tab = "Legit", Section = "Aim Assist", MinimumNumber = 0, MaximumNumber = 180, Suffix = "В°", Tooltip = "Controls how close from the center of your screen an enemy must be before the aim assist considers aiming at them"})
    ...
    UILibrary:CreateSlider({Name = "Deadzone FOV", Tab = "Legit", Section = "Aim Assist", MinimumNumber = 0, MaximumNumber = 50, Suffix = "В°", MinimumText = "Off", Tooltip = "Controls how close from the center of your screen an enemy must be before the aim assist stops aiming at them"})
    ...
    UILibrary:CreateSlider({Name = "Magnet FOV", Tab = "Legit", Section = "Trigger Bot", MinimumNumber = 0, MaximumNumber = 90, Suffix = "В°", Tooltip = "Controls the custom fov to be used for the aim assist on activation of the magnet triggerbot"})
    ...
    UILibrary:CreateSlider({Name = "Silent Aim FOV", Tab = "Legit", Section = "Bullet Redirection", MinimumNumber = 0, MaximumNumber = 180, Suffix = "В°", Tooltip = "Controls how close from the center of your screen an enemy must be before the silent aim considers aiming at them"})
    ...
    UILibrary:CreateSlider({Name = "Aimbot FOV", Tab = "Rage", Section = "Aimbot", Suffix = "В°", MinimumNumber = 0, MaximumNumber = 180, DefaultValue = 180, MaximumText = "Ignored", Tooltip = "Controls how close from the center of your screen an enemy must be before the aimbot considers aiming at them"})
    ...
    UILibrary:CreateSlider({Name = "Yaw angle", Tab = "Rage", Section = "Anti Aim", Suffix = "В°", MinimumNumber = -180, MaximumNumber = 180, Tooltip = "Fine tunes yaw angle."})
    UILibrary:CreateSlider({Name = "Yaw Jitter angle", Tab = "Rage", Section = "Anti Aim", Suffix = "В°", MinimumNumber = -180, MaximumNumber = 180, Tooltip = "Fine tunes yaw jitter angle."})
    ...
    UILibrary:CreateSlider({Name = "FOV", Tab = "Visuals", Section = "Camera", MinimumNumber = 10, MaximumNumber = 120, Suffix = "В°", DefaultValue = 80})
    ...
    UILibrary:CreateSlider({Name = "Pitch", Tab = "Visuals", Section = "Viewmodel", MinimumNumber = -180, MaximumNumber = 180, Suffix = "В°"})
    UILibrary:CreateSlider({Name = "Yaw", Tab = "Visuals", Section = "Viewmodel", MinimumNumber = -180, MaximumNumber = 180, Suffix = "В°"})
    UILibrary:CreateSlider({Name = "Roll", Tab = "Visuals", Section = "Viewmodel", MinimumNumber = -180, MaximumNumber = 180, Suffix = "В°"})
...
do --ANCHOR mathModule
    ...
    do -- ok ok heres trajectory
        ...
        function trajectory:CalculateSingle(x0, v0, ignoreList)
            ...
            until ((hit and hit.CanCollide == false) or t >= self.MaxTime)
...
do --ANCHOR Ragebot
    ...
    local oldIndex; oldIndex = hookmetamethod(game, "__index", function(self, k)
        if k == "Velocity" and self.Parent == localPlayer.Character and ragebot.realhrp then
            return oldIndex(ragebot.realhrp, k)
        end
    ...
    function ragebot.setupfakehrp(char)
        ...
        if not ragebot.realhrp or not char then 
            return
        end
    ...
    local fakeReplicationUpdate = function(uptime, delta)
        ...
        if not ragebot.realhrp or not localPlayer.Status.Alive.Value or not localPlayer.Character then
            return
        end
        ...
        if ragebot.realhrp then -- establish our un-fucked angles
            delayedcf = ragebot.realhrp.CFrame
        ...
        if Menu["Rage"]["Fake Lag"]["Enabled"]["Toggle"]["Enabled"] then
            if Menu["Rage"]["Fake Lag"]["Custom Triggers"]["Toggle"]["Enabled"] then
                if table.find(Menu["Rage"]["Fake Lag"]["Amount"]["Value"], "On Moving") then
                    if ragebot.realhrp.Velocity.Magnitude > 4 then
                        triggerfl = true
                    end
                end
                    ...
                    for i, v in next, (players:GetPlayers()) do
                        if v ~= localPlayer and trigger ~= true then
                            ...
                            if pInfo and pInfo.alive and pInfo.enemy and pInfo.character and pInfo.character:FindFirstChild("HumanoidRootPart") and workspace:FindFirstChild("Map") then
                                ...
                                local _, nextpos = workspace:FindPartOnRayWithIgnoreList(Ray.new(camera.CFrame.p, ragebot.realhrp.Velocity * (Menu["Rage"]["Fake Lag"]["Limit"]["Value"] / 256)), ignore, false, true)
        ...
        local viewheight = newVector3()

        if localPlayer.Character:FindFirstChild("Humanoid") then
            viewheight = localPlayer.Character.Humanoid.CameraOffset
        end

        local targetpos = triggerfl and delayedcf.p or ragebot.realhrp.Position
    ...
    function ragebot.fakeshoot(Parameters)
        ...
        if client.gun ~= "none" then
            --replicatedStorage.Weapons[client.gun.Name].Fire:Play()
            --fireAnimation:Play()
                ...
                if findFirstChild(localPlayer.Character.Gun, "Flash") and findFirstChild(camera, "Arms") and findFirstChild(camera.Arms, "Flash") then
                    ...
                    if Parameters.walls then
                        for i, v in next, (Parameters.walls) do
                            client.hitobject(v["Hit"], v["Enter"], v["Normal"], client.gun, false)
                            client.hitobject(v["Hit"], v["Exit"], -v["Normal"], client.gun, false)
                        end
                    end
                end
            end
        ...
        if Menu["Rage"]["Aimbot"]["Auto Shoot"]["Toggle"]["Enabled"] then
            --for i = 1, client.gun.Bullets.Value do
                ...
                --[[local args = {
                    ...
                    [3] = client.gun.Name, -- 3
                    [4] = 4096, --client.gun.Range.Value, -- 4
                    [5] = localPlayer.Character:FindFirstChild("Gun"), -- 5
                    [8] = 1,
                    [10] = false,
                    [11] = Vector3.new(),
                    [12] = 65,
    ...
    function ragebot.getpoints(enemy)
        ...
        local hitboxes, minimumDamage = ragebot.getweaponconfig(client.gun) -- lets get the current weapons config
    ...
    if not (localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") and Menu["Rage"]["Aimbot"]["Enabled"]["Toggle"]["Enabled"] and Menu["Rage"]["Aimbot"]["Enabled"]["Bind"]["Active"]) or not client.gun or client.gun == "none" or Menu["Misc"]["Extra"]["Kill All"]["Toggle"]["Enabled"] then
...
do --ANCHOR Legitbot
    ...
    legitbot.legitbotloop = runService.Stepped:Connect(function(delta)
        ...
        if not client.gun or client.gun == "none" or not localPlayer.Character or (localPlayer.Character and not localPlayer.Character:FindFirstChild("Humanoid")) then -- hmm i wonder where ive seen this b4
            ...
            legitbot.weaponstats.gunrange = client.gun.Range.Value
            legitbot.weaponstats.penetration = client.gun.Penetration.Value
                            ...
                            if pInfo.alive and pInfo.enemy then -- ok they are still alive
                                ...
                                if possiblepoints[hit] == true then -- ok so its still a valid point
                                    ...
                                    if Menu["Legit"]["Trigger Bot"]["Auto Wall"]["Toggle"]["Enabled"] then
                                        ...
                                        local weaponstat = {
                                            maxPenetration = client.gun.Penetration.Value * 0.01,
                                            maxWalls = 4
                                        }
...
do
    ...
    exploits.crashmessages = {
        "рџ–ҐпёЏрџ–ҐпёЏрџ–ҐпёЏ SERVER LOCKED рџ”’рџ”’рџ”’",
        "рџ–ҐпёЏрџ–ҐпёЏрџ–ҐпёЏ SERVER LOCKED BY BLOXSENSE рџ”’рџ”’рџ”’",
        "вЃпёЏвЃпёЏвЃпёЏ SERVER SMOKED вЃпёЏвЃпёЏвЃпёЏ",
        "вЃпёЏвЃпёЏвЃпёЏ SERVER SMOKED BY BLOXSENSE вЃпёЏвЃпёЏвЃпёЏ",
        "рџ–ҐпёЏрџ–ҐпёЏрџ–ҐпёЏ SERVER CLOSED рџ”’рџ”’рџ”’",
        "рџ‘‹рџ‘‹рџ‘‹ GOODBYE SERVER рџ‘‹рџ‘‹рџ‘‹",
        "рџ‘‹рџ‘‹рџ‘‹ GOODBYE рџ‘‹рџ‘‹рџ‘‹",
        "рџљЁрџљЁрџљЁ SERVER GONE рџљЁрџљЁрџљЁ",
        "рџљЁрџљЁрџљЁ SERVER LOCKED рџљЁрџљЁрџљЁ",
...
function exploits.crashserver() -- 30 seconds
        ...
        --[[if localPlayer and localPlayer.Character then
            if Menu["Misc"]["Exploits"]["God Mode Type"]["Value"] == "Hostage" then
                if localPlayer.Character:FindFirstChild("Hostage") then
                    ...
                    local real = replicatedStorage.Weapons:FindFirstChild(client.gun.Name)
            ...
            elseif Menu["Misc"]["Exploits"]["God Mode Type"]["Value"] == "Fall" then
                ...
                runService.RenderStepped:Connect(function()
                    for _,v in players:GetPlayers() do
                        if v:FindFirstChild("DamageLog") and not v.DamageLog:FindFirstChild(localPlayer.Name) then
    ...
    function exploits.cloneInstance(original: Instance): boolean
        ...
        if localPlayer.Character and client.gun then
            -- WOW! IPhone!!! WOW!!!!!
            local fakeGun = {}
            for i, v in next, client.gun:GetChildren() do
                fakeGun[v.Name] = v
            end
    ...
    function exploits.grabCharacter(character: Model): boolean
        ...
        if character and character:FindFirstChild("UpperTorso") and exploits.cloneInstance(character.Head) then
            ...
            localPlayer.Character.Gun:Destroy()
            replicatedStorage.Events.ApplyGun:FireServer(client.gun, localPlayer)
        end
